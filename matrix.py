...
print(connect_groups(read_xlsx('E:/Dropbox/OWEN/Matrix/пример.xlsx'),'тестовая2', 0))
#read_xlsx('E:/Programming/LabView/readExcel/пример.xlsx')
#get_group(read_xlsx('E:/Programming/LabView/readExcel/103-24.1618.17.2.0.xlsx'), 2,3)
#get_group(read_xls('E:/Programming/LabView/readExcel/test1.xls') ,13,156)
#connect_matrix("ab1", "card1r1", "эталоны")


#прошлие варианты функций:

# чтение xls, первой страницы и возврат данных в формате JSON
# функция стала неактуальной после перехода на xlsx!!!!!!!!!!!!!!!!!!!!!!!!!!
# def read_xls(file):
#     arr_groups = []
#     workbook = xlrd.open_workbook(file)
#     sheet1 = workbook.sheet_by_index(0)
#     nrows = sheet1.nrows
#     row = 1 #  начинать с первой строки, так как нулевая строка это шапка
#     while (row != nrows):
#         arr_groups.append({"name":sheet1.cell_value(row,0),
#                            "pin_number":sheet1.cell_value(row,1),
#                            "sensor":sheet1.cell_value(row,2),
#                            "code":json.loads(sheet1.cell_value(row,3))})
#         row+=1
#     return json.dumps(arr_groups, ensure_ascii=False)

# чтение xlsx страницы groups и возврат строки в формате JSON
# file(строка) - путь к файлу
# результат данной функции планируется обрабатывать в get_group() для выбора с какой строкой работать
# функция стала неактуальной после отказа от ключей!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# def read_xlsx(file):
#     arr_groups = []
#     workbook = pylightxl.readxl(file)
#     nrows = len(workbook.ws('groups').col(1))  # количество строк
...
#     while (row <= nrows):
#         arr_groups.append({"name":workbook.ws('groups').index(row=row, col=2),
#                            "pin_number": workbook.ws('groups').index(row=row, col=3),
#                            "sensor": workbook.ws('groups').index(row=row, col=4),
#                            "code": json.loads(workbook.ws('groups').index(row=row, col=10))})
#                            #"code": json.loads('[]')})
#         row += 1
#     return json.dumps(arr_groups, ensure_ascii=False)
